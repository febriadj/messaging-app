  ...
  return (
    ...
    >
          ...
          selectedParticipants.length > 0 && (
            <div className="relative flex items-center">
              ...
              <button
                ...
                onClick={() => {
                  const preview = document.querySelector('#select-participant #participants-preview');
                  preview.scrollTo({
              ...
              >
                <i className="pointer-events-none"><bi.BiChevronLeft /></i>
              </button>
              {/* preview */}
              <div id="participants-preview" className="py-2 px-4 flex gap-4 items-center overflow-x-auto scrollbar-none border-0 border-b border-solid border-spill-200 dark:border-spill-800">
                {
              ...
              >
                <i className="pointer-events-none"><bi.BiChevronRight /></i>
              </button>
            </div>
          )
        }
      ...
      <div className="pb-16 md:pb-0 overflow-y-auto scrollbar-thin scrollbar-thumb-spill-200 hover:scrollbar-thumb-spill-300 dark:scrollbar-thumb-spill-700 dark:hover:scrollbar-thumb-spill-600">
        {
          page.addParticipant && contacts && contacts
            .filter((elem) => !page.addParticipant.participantsId.includes(elem.friendId))
            .map((elem) => (
              <div
                key={elem._id}
                aria-hidden
                ...
                onClick={(e) => {
                  ...
                  const isSelected = selectedParticipants.find(({ _id }) => _id === elem._id);
                  dispatch(setSelectedParticipants(
                    isSelected
                      ? selectedParticipants.filter(({ _id }) => _id !== elem._id)
      ...
      <button
        type="button"
        className={`
          ${(modal.confirmAddParticipant || selectedParticipants.length <= 0) && 'scale-0 opacity-0 -z-10'}
      ...
      >
        <i><bi.BiRightArrowAlt size={28} /></i>
      </button>
    </div>
  );
}
