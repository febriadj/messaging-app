...
function EditGroup() {
  ...
  const handleSubmit = (e) => {
    ...
    if (form.name === modal.editGroup.name && form.desc === modal.editGroup.desc) {
      handleCloseModal();
    }
  ...
  return (
    ...
    >
      <div
        aria-hidden
        className={`${!modal.editGroup && 'scale-0'} transition w-[460px] m-6 p-4 grid rounded-md bg-white dark:bg-spill-800`}
        onClick={(e) => {
          e.stopPropagation();
        }}
      >
          ...
          modal.editGroup && (
            <form method="post" className="mt-4 grid" onSubmit={handleSubmit}>
              ...
              <span className="flex gap-2 mt-4 justify-end">
                ...
                <button type="submit" className="py-2 px-4 rounded-md bg-sky-600 hover:bg-sky-700">
