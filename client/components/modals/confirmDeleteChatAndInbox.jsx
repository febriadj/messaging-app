...
function ConfirmDeleteChatAndInbox() {
  ...
  const confirmDeleteChatAndInbox = useSelector((state) => state.modal.confirmDeleteChatAndInbox);
  ...
  const handleDeleteChatAndInbox = async () => {
    try {
      ...
      if (confirmDeleteChatAndInbox.inboxId === chatRoom.isOpen && chatRoom.data._id) {
  ...
  return (
    <div
      className={`
        ${confirmDeleteChatAndInbox ? 'delay-75 z-50' : '-z-50 opacity-0 delay-300'}
    ...
    >
      <div
        aria-hidden
        className={`${!confirmDeleteChatAndInbox && 'scale-0'} transition relative w-[400px] m-6 p-4 rounded-md bg-white dark:bg-spill-800`}
        onClick={(e) => {
          e.stopPropagation();
        }}
      >
        ...
        <span className="mt-4 flex gap-2 justify-end">
          <button
            ...
            onClick={() => {
              dispatch(setModal({ target: 'confirmDeleteChatAndInbox', data: false }));
            }}
          >
            <p>Cancel</p>
          </button>
