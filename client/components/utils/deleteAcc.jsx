...
function DeleteAccount() {
  ...
  return (
    ...
    >
      <div
        aria-hidden
        className={`${!modal.deleteAcc && 'scale-0'} transition w-[460px] m-6 p-4 grid rounded-md bg-white dark:bg-spill-800`}
        onClick={(e) => {
          e.stopPropagation();
        }}
      >
        ...
        <blockquote className="my-2 p-2 border-0 border-l-2 border-solid border-rose-600 bg-rose-50 dark:bg-rose-400/20">
          <p className="text-rose-900 dark:text-rose-100">This is extremely important.</p>
        </blockquote>
        <p className="mb-4">Once your account is deleted, all of your data will be permanently gone, including your profile, contacts, and chats.</p>
        ...
        <label htmlFor="username" className="mt-2 relative flex items-center">
          <input
            ...
            minLength="6"
            className={`${password.length > 0 ? 'peer valid:bg-spill-50 dark:valid:bg-spill-900' : ''} w-full py-2 pl-4 pr-12 border border-solid border-spill-300 dark:border-spill-500 rounded-md focus:border-black dark:focus:border-sky-400`}
            value={password}
        ...
        { respond.message && (
          <p className={`${!respond.success && 'text-rose-800 dark:text-rose-400'} text-sm mt-2`}>
            {respond.message}
          </p>
        ) }
